    ...
    DB.get_or_init(|| {
        ...
        rt.block_on(async {
            let db_url = env::var("SURREAL_URL")
                .with_context(|| "SURREAL_URL must be set")
                .unwrap();
            let db_user = env::var("SURREAL_USER")
                .with_context(|| "SURREAL_USER must be set")
                .unwrap();
            let db_pass = env::var("SURREAL_PASS")
                .with_context(|| "SURREAL_PASS must be set")
                .unwrap();
            let db_name = env::var("SURREAL_DB")
                .with_context(|| "SURREAL_DB must be set")
                .unwrap();
            let db_ns = env::var("SURREAL_NS")
                .with_context(|| "SURREAL_NS must be set")
                .unwrap();
